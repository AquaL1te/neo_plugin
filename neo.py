...
class LookupModule(LookupBase):
    def run(self, terms, variables, **kwargs):
        def _raise_terms_error(msg=""):
            raise AnsibleError(
                "neo lookup expects: data (list of dicts), select value, filter value, [{'select_key': 'selector'}, where selector is by default 'name']. "
                + msg
            )
        ...
        if (
            not isinstance(items, (list, dict))
            or not isinstance(select_value, string_types)
            or not isinstance(filter_value, string_types)
        ):
            _raise_terms_error()
        ...
        if len(terms) == 4:
            ...
            if not isinstance(flags, dict) and not all(
                ...
                _raise_terms_error(
                    "The optional fourth item must be a dict with flags '{}'.".format(
                        FLAGS
                    )
                )

            # parsing flags
            if flags.get("select_key", False) and isinstance(
                flags["select_key"], string_types
            ):
                select_key = flags["select_key"]
                display.vvv(
                    "Filter key default 'name' overridden with '{}'.".format(select_key)
                )
            else:
        ...
        for item in items:
            ...
            if selected_key == select_value:
                ...
                if filter_value in item:
                    display.vvv(
                        "Found filter match for '{}: {}' in data subset '{}: {}'.".format(
        ...
        _raise_terms_error(
            "Failed to find filter match for '{}: {}' in data subset '{}'.".format(
                select_key if "select_key" in locals() else "name",
                select_value,
                filter_value,
            )
